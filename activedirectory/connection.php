...
namespace ActiveDirectory {
	...
	class Connection {
		...
		public function connect() {
			if(!$this->conn) {
				...
				if(!ldap_bind($this->conn, $this->user, $this->password)) {
					throw new ConnectionException("Login to '{$this->host}' using '{$this->user}' failed: " . ldap_error($this->conn));
				}
			}
			return $this->conn;
		}
		...
		public function query($query, array $attributes = null, $base_dn = null) {
			...
			if($result === false) {
				throw new ConnectionException("Executing query '$query' failed: " . ldap_error($this->conn));
			}
